...
void gravarDados( void )
{
	FILE * Arq;
	/* Abre o arquivo */	
	Arq = fopen ("E:\\Drive_Thru_E\\ARQUIVOS\\PRODUTOS.DAT", "a");
...
void cadastroDefault (void)
{
	FILE *Arq;
	int i;
	Arq = fopen ("E:\\Drive_Thru_E\\ARQUIVOS\\PRODUTOS.DAT" , "w");
	if ( Arq == NULL)
	{
...
void gerarRelatorio(void)
{
    ...
    Arq = fopen("E:\\Drive_Thru_E\\ARQUIVOS\\PRODUTOS.DAT", "r"); /* Lê o Produtos.Dat*/
    if (Arq == NULL)
    {
    ...
    Relat = fopen("E:\\Drive_Thru_E\\ARQUIVOS\\RELATPRODUTOS.TXT", "w"); /* Sobrescreve o Relat.txt*/
    if (Relat == NULL)
    {
